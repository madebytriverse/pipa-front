...
import { useProducts, type Category } from "../../../store/infrastructure/useProducts";
import { useCoupons } from "../../../admin/infrastructure/useCoupons";

interface SellerCouponCRUDModalProps {
    coupon?: any;
    onClose: () => void;
...
export default function SellerCouponCRUDModal({
    coupon,
    onClose,
    onSave,
}: SellerCouponCRUDModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (coupon && coupon.id) {
                result = await updateCoupon(coupon.id, cleanedData);
            } else {
                result = await createCoupon(cleanedData);
            }
            ...
            // 🔹 Opcional si usás useAlert():
            // showAlert("Error", err.response?.data?.message || "No se pudo guardar el cupón.", "error");
        }
    };
